        ...
        <form action="<?= base_url('admin/jadwal/update') ?>" method="POST">
          ...
          <div class="card-body col-6">
              ...
              <label for="nama_kereta">Nama Kereta *</label>
                <select name="nama_kereta" id="nama_kereta" class="form-control select2">
                  <?php foreach ($kereta as $value): ?>
                    <option value="<?= $value['id'] ?>" <?= $value['id'] == $jadwal['id_kereta'] ? 'selected' : null ?> ><?= $value['nama_kereta'] . ' ('.$value['tipe_kereta'].')' ?></option>
                  <?php endforeach ?>
                </select>
            ...
            <div class="form-group m-t-20">
              ...
              <input type="date" name="tanggal" id="tanggal" class="form-control" min="<?= date('Y-m-d') ?>" value="<?= $jadwal['tanggal'] ?>">
              <strong class="text-danger"><?= form_error('tanggal') ?></strong>
            </div>
            <div class="row">
              <div class="form-group col-md-6">
                <label for="waktu_berangkat">Waktu Berangkat *</label>
                <input type="time" name="waktu_berangkat" id="waktu_berangkat" class="form-control" value="<?= $jadwal['waktu_berangkat'] ?>">
              ...
              <div class="form-group col-md-6">
                ...
                <input type="time" name="waktu_sampai" id="waktu_sampai" class="form-control" value="<?= $jadwal['waktu_sampai'] ?>">
              </div>
            </div>
            <div class="form-group m-t-20">
              <label for="harga">Harga *</label>
              <input type="number" name="harga" id="harga" class="form-control" value="<?= $jadwal['harga'] ?>">
